...
function getAllClassInfo_callback(res) {
    if(res.code==200){
        ...
        if(data.length>0){
            ...
            $("#allClass >option[value="+orgid+"]").val(orgid).siblings().remove();
            getClassMemberInfo_port(orgid);
        }
...
function getSingleHealthInfo_port(orgid) {
    var data={
            loginuuid:user.curCompany.id,
            hiuuid:GetQueryString("hiuuid")
        };
    var param={
            params:JSON.stringify(data)
    };
    initAjax(httpUrl.getSingleHealthInfo,param,getSingleHealthInfo_callback);
};
function getSingleHealthInfo_callback(res) {
    if(res.code==200){
        var data=JSON.parse(res.data);
        $("#classMember >option[value="+data.useruuid+"]").val(data.useruuid).siblings().remove();
        $("#createDate").val(data.examdateStr);
...
function insertHealthInfo(){
    ...
    initAjax(httpUrl.updateHealthInfo,param,insertHealthInfo_callback);
}
