...
function loginSuccess() {
    ...
    // 获取子集文件UUID
$("#todolist-wrap").on('click','.todolist li',function() {
...
}).on("dblclick",".todolist >li:not(.folder)",function () {
    ...
    if(num == "text"){
        var src = httpUrl.fileDownload+'?params='+encodeURIComponent('{"fileMd5":"'+$(this).attr('data-filemd5')+'","fileName":"'+$(this).find("span").html()+'"}'); 
        window.open("http://dcsapi.com/?k=106194529&url="+encodeURIComponent(src));
    }else if(num == "image"){
        $("#carousel_img").empty().append("<img src="+httpUrl.path_img+$(this).attr('data-filemd5')+">");
...
$("#preview").click(function() {
    ...
    if(liarray.length==0){
        ...
        alert("请选择一个文件下载。");
        $("#todolist-wrap .todolist li.active").removeClass("active")
    }else{ 
        try{ 
