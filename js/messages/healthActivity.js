...
function loginSuccess(res) {
    ...
    GetHealthColumnInfos_port();// 获取课程计划列表
...
// 查询接口函数
function GetHealthColumnInfos_port(pageNumber) {
    var data={
    ...
    initAjax(httpUrl.GetHealthColumnInfos,param,GetHealthColumnInfos_callback);
};
function GetHealthColumnInfos_callback(res) {
    if(res.code==200){
        ...
        $("#pagination").pagination({
            ...
            onPageClick: function (NowPage, event) {
                GetHealthColumnInfos_port(NowPage);
            }
        });
    }else{
...
function classActivitySaveBtn(res) {
    ...
    var classId=res[3];
    var urlSearch="healthActivity_new.html"+"?userUuid="+userUuid+"&classId="+classId+"&perssionNames=通知管理健康专栏";
...
function pagination01() {
    $("#email-content").on("click","#pagination01 >li:not(.disabled) >a",function () {
        ...
        GetHealthColumnInfos_port(pageNumber);
    });
}
