...
// 查询接口函数
function AddNewSchoolInfo_port() {
    var notifygroup=[];
    var notifyperson=[];
    $.each($("#allClass input"),function (index,value) {
    ...
    if(!data.link && !ue.getContent()){
        ...
        initAjax(httpUrl.AddNewSchoolInfo,param,AddNewSchoolInfo_callback);
    }
};
function AddNewSchoolInfo_callback(res) {
    if(res.info=="OK"){
        window.location.href="schoolActivity.html"+"?userUuid="+user.userUuid+"&classId="+user.curClassId+"&perssionNames=通知管理学校通知";
    }else{
        alert(res.info);
    }
};
