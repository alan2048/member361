...
function loginSuccess(res) {
    ...
    GetParentsLessionInfos_port();// 获取课程计划列表
...
// 查询接口函数
function GetParentsLessionInfos_port(pageNumber) {
    var data={
    ...
    initAjax(httpUrl.GetParentsLessionInfos,param,GetParentsLessionInfos_callback);
};
function GetParentsLessionInfos_callback(res) {
    if(res.code==200){
        ...
        $("#pagination").pagination({
            ...
            onPageClick: function (NowPage, event) {
                GetParentsLessionInfos_port(NowPage);
            }
        });
    }else{
...
function classActivitySaveBtn(res) {
    ...
    var classId=res[3];
    var urlSearch="homeActivity_new.html"+"?userUuid="+userUuid+"&classId="+classId+"&perssionNames=通知管理家长课堂——新增";
...
function pagination01() {
    $("#email-content").on("click","#pagination01 >li:not(.disabled) >a",function () {
        ...
        GetParentsLessionInfos_port(pageNumber);
    });
}
