...
// 查询接口函数
function AddHealthColumnInfo_port() {
    var notifygroup=[];
    var notifyperson=[];
    $.each($("#allClass input"),function (index,value) {
    ...
    if(!data.link && !ue.getContent()){
        ...
        initAjax(httpUrl.AddHealthColumnInfo,param,AddHealthColumnInfo_callback);
    }
};
function AddHealthColumnInfo_callback(res) {
    if(res.info=="OK"){
        window.location.href="healthActivity.html"+"?userUuid="+user.userUuid+"&classId="+user.curClassId+"&perssionNames=通知管理健康专栏";
    }else{
        alert(res.info);
    }
};
