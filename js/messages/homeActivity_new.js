...
// 查询接口函数
function AddParentsLessionInfo_port() {
    var notifygroup=[];
    var notifyperson=[];
    $.each($("#allClass input"),function (index,value) {
    ...
    if(!data.link && !ue.getContent()){
        ...
        initAjax(httpUrl.AddParentsLessionInfo,param,AddParentsLessionInfo_callback);
    }
};
function AddParentsLessionInfo_callback(res) {
    if(res.info=="OK"){
        window.location.href="homeActivity.html"+"?userUuid="+user.userUuid+"&classId="+user.curClassId+"&perssionNames=通知管理家长课堂";
    }else{
        alert(res.info);
    }
};
