...
function loginSuccess(res) {
    ...
    GetSchoolNotifyInfos_port();// 获取课程计划列表
...
// 查询接口函数
function GetSchoolNotifyInfos_port(pageNumber) {
    var data={
    ...
    initAjax(httpUrl.GetSchoolNotifyInfos,param,GetSchoolNotifyInfos_callback);
};
function GetSchoolNotifyInfos_callback(res) {
    if(res.code==200){
        ...
        $("#pagination").pagination({
            ...
            onPageClick: function (NowPage, event) {
                GetSchoolNotifyInfos_port(NowPage);
            }
        });
    }else{
...
function classActivitySaveBtn(res) {
    ...
    var classId=res[3];
    var urlSearch="schoolActivity_new.html"+"?userUuid="+userUuid+"&classId="+classId+"&perssionNames=通知管理学校通知";
...
function pagination01() {
    $("#email-content").on("click","#pagination01 >li:not(.disabled) >a",function () {
        ...
        GetSchoolNotifyInfos_port(pageNumber);
    });
}
