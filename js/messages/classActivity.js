...
function loginSuccess(res) {
    ...
    GetClassNotifyInfos_port();// 获取课程计划列表
...
// 查询接口函数
function GetClassNotifyInfos_port(pageNumber,classId) {
    var data={
            req_username:user.userUuid,
            req_class_id:$("#userClass").val() || "",
            req_page_num:pageNumber || 1
        };
    var param={
            params:JSON.stringify(data)
    };
    initAjax(httpUrl.GetClassNotifyInfos,param,GetClassNotifyInfos_callback);
};
function GetClassNotifyInfos_callback(res) {
    if(res.code==200){
        var data=JSON.parse(res.data);
        ...
        $("#pagination").pagination({
            ...
            onPageClick: function (NowPage, event) {
                GetClassNotifyInfos_port(NowPage);
            }
        });
    }else{
        // console.log('请求错误，返回code非200');
...
function classActivitySaveBtn(res) {
    ...
    var classId=res[3];
    var urlSearch="classActivity_new.html"+"?userUuid="+userUuid+"&classId="+classId+"&perssionNames=通知管理班级活动";
...
function pagination01() {
    $("#email-content").on("click","#pagination01 >li:not(.disabled) >a",function () {
        ...
        GetClassNotifyInfos_port(pageNumber);
    });
}